      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-16">
        <div className="text-center mb-10">
          ...
          <div className="text-lg text-muted-foreground">
            <p>{dict.main.description}</p>
            <p>{dict.main.description2}</p>
          </div>
        </div>
        ...
        <div className="bg-background rounded-2xl shadow-lg p-8 mb-8 border border-border transition-all duration-300 hover:shadow-xl">
          <div className="relative">
            <Textarea
              ...
              maxLength={5000}
              placeholder={dict.main.input.placeholder}
              readOnly={isLoading || !!url}
              className="w-full h-56"
            />
            <div className="absolute bottom-4 right-4 text-sm text-muted-foreground">
              {text.length}/ 5000 {dict.main.input.length}
            </div>
          </div>
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8">
            <div className="flex gap-3">
              ...
              >
                {dict.main.option.button}
              </Button>
              <Button variant="outline" size="lg" onClick={() => setIsOption(!isOption)}>
                {isOption ? dict.main.option.option2 : dict.main.option.option}
              </Button>
            </div>
